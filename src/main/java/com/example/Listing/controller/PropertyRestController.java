...
public class PropertyRestController {
    ...
    public void bulkUpdate(@RequestParam("batch") int batch) { // List<String> prpertyId ; cityId
        String PType = "Rent";
        List<QualityScore> PropArr = propertyRepository.findAll();
        ExecutorService executorService = Executors.newCachedThreadPool();
        for (int i = 0, l = PropArr.size(); i < l; i += batch) {
            try {
                QualityScore qualityScore = propertyService.calculateQualityScore("4", PropArr.get(i).getPropertyId(), PType); // todo : relevance score
